					...
					success : function(message) {
						if (message != null) {
							...
							window.location.href = "st-check.html?projectNum="
									+ message.integralLogId.projectNum;
						}
					},
...
function addHtmlss(result, method) {
	...
	for (var i = 0; i < result.length; i++) {
		if (result[i].status === "PENDING_AUDIT_BEFORE_FAIL"
				|| result[i].status === "PENDING_AUDIT_FAIL"
				|| result[i].status === "PENDING_AUDIT_AGAIN_FAIL"
				|| result[i].status === "PENDING_AUDIT_FINAL_FAIL") {
			url = "st-detail4.html";
		}
		htmls += "<div class='act-div'><div class='tab-left'><img src='"
				+ result[i].integralLogPic + "'></div>";
		htmls += "<div class='tab-right'><div class='tab-title'><div class='right-name'>"
				+ subMyStr(result[i].projectName) + "</div></div>";
		htmls += "<div class='tab-text'><span>" + subSmallStr(result[i].event)
				+ "</span></div>";
		htmls += "<a href='" + url + "?projectNum="
				+ result[i].integralLogId.projectNum
				+ "'><div class='tab-search'>" + "查看详情"
				+ " ></div></a></div></div>";
	}
	if (method === "inreview") {
		if (page == 0) {
