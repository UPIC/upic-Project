...
$(function(){
    ...
    $(".btn-primary").click(function() {
        if ($('table tr:last>td').length==1) {
          $('table tr:last').append('<td><span>编辑类别名</span><button type='submit' class='btn btn-small'>编辑</button></td>');
          var cn= $('table tr:last>td').find('span').html();
          addProjectCategory(cn);
          getDate();
      }else if($('table tr:last>td').length==2){
          $('table tr:last').after('<tr><td><span>编辑类别名</span><button type='submit'  class='btn btn-small'>编辑</button></td></tr>');
          var cn= $('table tr:last>td').find('span').html();
          addProjectCategory(cn);
          getDate();
      }
