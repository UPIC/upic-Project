...
$(function() {
	...
	$("#exportBtn").click(
			function() {
				var baseModels = [ "projectNum", "studentNum", "event",
						"integral", "projectName", "student",
						"projectCategory", "college", "clazz", "status" ];
				var str = JSON.stringify(baseModels);
				var form = $("<form></form>").attr("action", exportExcelUrl)
						.attr("method", "GET");
				form.append($("<input></input>").attr("type", "hidden").attr(
						"name", "baseModel").attr("value", str));
				var formKeyValue = appendForm(requestData);
				for (var i = 0; i < formKeyValue.key.length; i++) {
					form.append($("<input></input>").attr("type", "hidden")
							.attr("name", formKeyValue.key[i]).attr("value",
									formKeyValue.value[i]));
				}
...
function addHtmls(datas, pageNum) {
	...
	for (var i = 0; i < data.length; i++) {
		...
		htmls += "<td class='center_td'><button class='btn btn-mini btn-"
				+ color + "'>" + statusC + "</button></td>";
		htmls += "<td>" + splitJson(data[i].event) + "</td>";
		htmls += "<td>" + data[i].projectName + "</td>";
		htmls += "<td>" + data[i].college + "</td>";
		htmls += "<td>" + data[i].integral + "</td>";
		htmls += "<td>";
		htmls += "<a href='#mymodal1' data-toggle='modal'><div class='message_div' onclick=commonAjax('"
				+ getIntegralLogByIntegralLogId
				+ "','projectNum="
				+ data[i].integralLogId.projectNum
				+ "','getProjectInfo','GET','"
				+ (i + 1)
				+ "')>查看详情</div></a></td>";
		htmls += "</tr>";
...
function getProjectInfo(result, j) {
	...
	htmlss += "<div class='span3'>" + result.integralLogId.projectNum
			+ "</div>";
	htmlss += "</div>";
	htmlss += "<div class='row-form clearfix'>";
	htmlss += "<div class='span3'>项目申请日期</div>";
	htmlss += "<div class='span3'>"
			+ getDate(result.creatTime, "yyyy-MM-dd hh:mm") + "</div>";
	...
	htmlss += "<div class='span3' id='guidanceMan"
			+ result.integralLogId.projectNum + "'>" + "1" + "</div>";
	htmlss += "</div>";
	htmlss += "<div class='row-form clearfix'>";
	htmlss += "<div class='span3'>项目内容</div>";
	htmlss += "<div class='span9' id='content"
			+ result.integralLogId.projectNum + "'>" + "1" + "</div>";
	htmlss += "</div>";
	htmlss += "<div class='row-form clearfix'>";
	htmlss += "<div class='span3'>评价标准与形式</div>";
	htmlss += "<div class='span9' id='checkAssessmentCriteraAndForm"
			+ result.integralLogId.projectNum + "'>" + "1" + "</div>";
	htmlss += "</div>";
