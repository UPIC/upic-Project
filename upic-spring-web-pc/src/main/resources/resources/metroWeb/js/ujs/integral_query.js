...
function addStatus(res) {
	...
	for (var i = 0; i < data.length; i++) {
		htmls += "<option value='" + (i + 4) + "'>" + data[i].status
				+ "</option>";
	}
...
function addHtmls(datas, pageNum) {
	...
	for (var i = 0; i < data.length; i++) {
		...
		htmls += "<tr><td><input type='checkbox' class='checkboxes' value='1' id='"
				+ data[i].integralLogId.projectNum + "'/></td>";
		htmls += "<td class='center_td'>"
				+ (parseInt(pageNum) * parseInt(pageSize) + i + 1) + "</td>";
		...
		htmls += "<td class='center_td'>";
		htmls += "<div class='message_div' onclick=commonAjax('"
				+ getIntegralLogByIntegralLogId + "','projectNum="
				+ data[i].integralLogId.projectNum
				+ "','getProjectInfo','GET')><a href='" + aUrl
				+ "' data-toggle='modal'>查看详情</a></div></td>";
		htmls += "</tr>";
	}
	$("#data").html(htmls);
...
function getProjectInfo(result) {
	...
	if (result.status === "FAILURE_TO_PASS_THE_AUDIT") {// 审核失败
		...
		htmlss += "<div class='span3'>"
				+ (parseInt(pageNum) * parseInt(pageSize) + i) + "</div>";
		...
		htmlss += "<div class='span3'>"
				+ getDate(result.creatTime, "yyyy-MM-dd") + "</div>";
		...
		htmlss += "<div class='span3'>编号</div>";
		htmlss += "<div class='span3'>"
				+ (parseInt(pageNum) * parseInt(pageSize) + i + 1) + "</div>";
		htmlss += "<div class='span3'>代码</div>";
		htmlss += "<div class='span3'>" + result.integralLogId.projectNum
				+ "</div>";
		htmlss += "</div><div class='row-form clearfix'>";
