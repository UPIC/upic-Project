...
*/
var getProjectByUser="/common/getProjectByUser";//查看我的项目列表
...
$(function () {
    /*
     加载数据
     */
    ajaxs("guidanceNum", "showAll", getProjectByUser)
...
function addHtmls(result,method) {
    ...
    if (method==="showAll") {
        for (var i=0;i<=result.content.length;i++) {
            ...
            if (result.content[i].implementationProcess=="SAVED") {
                status="已保存"
            }else if (result.content[i].implementationProcess==) {
                status="审核中"
            }else if (result.content[i].implementationProcess==) {
                status="未通过"
            }else if (result.content[i].implementationProcess==) {
    ...
    $("#apply").html(htmls);
         ...
         for (var i=0;i<=result.length;i++) {
            ...
            if (result[i].implementationProcess=="SAVED") {
                statuss="已保存"
            }else if (result[i].implementationProcess==) {
                statuss="审核中"
            }else if (result[i].implementationProcess==) {
                statuss="未通过"
            }else if (result[i].implementationProcess==) {
