...
public class ProjectServiceImpl implements ProjectService {
    ...
    public Page<ProjectInfo> getProjectWithoutSignUp(Date now, Pageable pageable) {
        ...
        try {
            ...
            return QueryResultConverter.convert(projectPage, pageable,
                    new AbstractDomain2InfoConverter<Project, ProjectInfo>() {
                        protected void doConvert(Project domain, ProjectInfo info) throws Exception {
                            UpicBeanUtils.copyProperties(domain, info);
                        }
    ...
    @Override
    public Page<ProjectInfo> getProjectBySql(List<String> statusList, List<String> projectCategoryList,
                                             Pageable pageable, String rank, String colloge, String type) {
        Page<Project> projectPage = null;
        try {
    ...
    public List<Object> exportProjectByGuidanceNum(String guidanceNum) {
        ...
        try {
            ...
            List<ProjectInfo> convert = QueryResultConverter.convert(projectList,
                    new AbstractDomain2InfoConverter<Project, ProjectInfo>() {
                        @Override
                        protected void doConvert(Project domain, ProjectInfo info) throws Exception {
                            UpicBeanUtils.copyProperties(domain, info);
...
//	@Scheduled(cron = "0 0/2 * * * ? ")
    ...
    public String qrCodeGenerate(String projectNum, long freshTime) {
        try {
            ...
            System.out.println("projectNum:" + projectNum + "nowTime:" + nowTime + " accessToken:" + accessToken);
            // 存Redis
            // UpicRedisComponent upicRedisComponent = new UpicRedisComponent();
            redisService.set("QR" + projectNum, accessToken, freshTime);
    ...
    public String deleteProject(Long id) {
        try {
            ...
            LOGGER.info("deleteProject。错误信息：" + e.getMessage());
            return null;
        }
    }
}
