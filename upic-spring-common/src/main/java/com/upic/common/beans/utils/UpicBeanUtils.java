...
package com.upic.common.beans.utils;
 ...
 * @author Sam Brannen
 */
public abstract class UpicBeanUtils {

	private static final Log logger = LogFactory.getLog(UpicBeanUtils.class);

	private static final Set<Class<?>> unknownEditorTypes = Collections
			.newSetFromMap(new ConcurrentReferenceHashMap<Class<?>, Boolean>(64));
	...
	// return new MethodParameter(((GenericTypeAwarePropertyDescriptor)
	// pd).getWriteMethodParameter());
	// }
	// else {
		...
		for (PropertyDescriptor targetPd : targetPds) {
			...
			if (writeMethod != null && (ignoreList == null || !ignoreList.contains(targetPd.getName()))) {
				...
				if (sourcePd != null) {
					...
					} else if (targetPd.getReadMethod().getName().equals(readMethod.getName())) {
						try {
							Object returnType = writeMethod.getParameterTypes()[0].newInstance();
							Object value = readMethod.invoke(source);
							if (returnType != null && value != null) {
								copyProperties(value, returnType, null, (String[]) null);
								writeMethod.invoke(target, returnType);
							}
						} catch (Throwable ex) {
							throw new FatalBeanException(
									"Could not copy property '" + targetPd.getName() + "' from source to target", ex);
