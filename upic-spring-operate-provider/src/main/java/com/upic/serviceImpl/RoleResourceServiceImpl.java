...
public class RoleResourceServiceImpl implements RoleResourceService {
	...
	@Override
	public List<RoleResourceInfo> getByRoleId(long roleId) {
		List<RoleResourceInfo> roleResourceInfoList = new ArrayList<>();
		List<RoleResource> roleResourcesList = null;
		try {
			roleResourcesList = roleResourceRepository.getByRoleId(roleId);
			for (RoleResource roleResource : roleResourcesList) {
				RoleResourceInfo roleResourceInfo = new RoleResourceInfo();
				UpicBeanUtils.copyProperties(roleResource, roleResourceInfo);
				roleResourceInfoList.add(roleResourceInfo);
			}
			return roleResourceInfoList;
		} catch (Exception e) {
			LOGGER.info("getByRoleId：" + e.getMessage());
			return null;
		}
	}
	...
	public String updateRoleResource(List<RoleResourceInfo> roleResourceInfoList,
			List<RoleResourceInfo> beforeRoleResourceInfoList) {
		beforeRoleResourceInfoList.stream().parallel().forEach(x -> {
			RoleResource roleResource = new RoleResource();
			UpicBeanUtils.copyProperties(x, roleResource);
			roleResourceRepository.delete(roleResource);
		});
		roleResourceInfoList.stream().parallel().forEach(x -> {
			RoleResource roleResource = new RoleResource();
			UpicBeanUtils.copyProperties(x, roleResource);
