...
//            integralLogId.setStudentNum(studentNum);
	...
	public IntegralLogInfo signUp(IntegralLogInfo integralLogInfo, int allNum) {
		try {
			Long num = redisComponent.increment(integralLogInfo.getIntegralLogId().getProjectNum());
			if (num > allNum) {
				throw new Exception("1001");
			}
			if(!redisComponent.putIfAbsent(integralLogInfo.getIntegralLogId().getProjectNum()+"hash",integralLogInfo.getIntegralLogId().getStudentNum(), num+"")) {
				throw new Exception("1002");
			}
			integralLogInfo.setField1(num+"");
			...
			redisComponent.decrement(integralLogInfo.getIntegralLogId().getProjectNum());
			return null;
		}
